	...
	useEffect(() => {
		const submitRegistration = async () => {
			try {
				...
				const formData = verifyData.formData
				...
				Object.entries(formData).forEach(([key, value]) => {
					if (typeof value === 'boolean') {
						...
						formDataToSend.append(key as string, value as string)
					}
				})
				...
				setStatus('error')
				setMessage(error instanceof Error ? error.message : 'Failed to submit registration')
			}
		}
				...
				<div style={{
					...
					{status === 'processing' && (
						<>
							...
							`}</style>
							<h2 style={{ color: '#0e062e', marginBottom: '15px' }}>Processing Your Registration</h2>
					...
					{status === 'error' && (
						<>
							...
							<h2 style={{ color: '#dc3545', marginBottom: '15px' }}>Registration Error</h2>
